...
impl Glyph {
    ...
    pub fn new<T: Into<String>>(name: T, codepoint: char) -> Self {
        Glyph { name: Some(name.into()), codepoint: Some(codepoint), ..Default::default() }
    }

    /// Validates the definition.
    pub fn validate(&self) -> bool {
        if self.name.is_none() { return false; }
        if self.codepoint.is_none() { return false; }
        if self.bounds.is_none() { return false; }
        if self.direction == Direction::Default {
            if self.alternate_scalable_width.is_some() { return false; }
            if self.alternate_device_width.is_some() { return false; }
        } else {
            if self.alternate_scalable_width.is_none() { return false; }
            if self.alternate_device_width.is_none() { return false; }
        }
        true
    }
    ...
    /// Gets the codepoint.
    pub fn codepoint(&self) -> char { self.codepoint.unwrap() }

    /// Sets the codepoint.
    pub fn set_codepoint(&mut self, codepoint: char) { self.codepoint = Some(codepoint); }
    ...
    /// Gets the bitmap.
    pub fn map(&self) -> &Bitmap { &self.map }

    /// Sets the bitmap.
    pub fn set_map(&mut self, map: Bitmap) { self.map = map; }

    /// Create an iterator over the pixels which will yield `((x, y), value)`.
    pub fn pixels(&self) -> PixelIter { PixelIter { x: 0, y: 0, map: &self.map } }
}
